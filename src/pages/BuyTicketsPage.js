...
const BuyTicketsPage = () => {
  ...
  const initiatePayment = async () => {
    ...
    try {
      const orderRes = await fetch("https://dyp-tedx-backend-server.onrender.com/api/payment/create-order", {
        method: "POST",
  ...
  const verifyPayment = async (response) => {
    try {
      const res = await fetch("https://dyp-tedx-backend-server.onrender.com/api/payment/verify-payment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
